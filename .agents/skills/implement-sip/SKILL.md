---
name: implement-sip
description: Implement SIP specification changes using repository conventions.
---

# Implement SIP

Patterns for implementing spec changes in `src/sila/forks/`. Run this skill before implementing an SIP or modifying fork code.

## Fork Directory Layout

Each fork lives at `src/sila/forks/<fork_name>/`. Explore the latest fork directory for current structure. Key files:
...
## Import Isolation (enforced by `sila-spec-lint`)

- **Within same fork**: relative imports (`from . import vm`, `from .state import ...`)
- **Previous fork only**: absolute imports (`from sila.cancun import ...`)
- **Shared modules**: always OK (`sila.crypto`, `sila.utils`, `sila.exceptions`)
- **Future forks**: NEVER allowed
- **Ancient forks (2+ back)**: NEVER allowed
- Run `sila-spec-lint` to verify before committing
...
6. Avoid "frame" in gas identifiers (a future SIP claims the term); when a name diverges from the spec's variable name, cross-reference the spec name in the docstring.
...
```bash
uv run sila-spec-new-fork --new-fork=<name> --template-fork=<template>
```
...
- Feature branches: `sips/<fork_name>/sip-<number>`
