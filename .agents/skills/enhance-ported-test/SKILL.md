...
was capped by SIP-3529 at `97857 // 5 = 19571`, giving the reported 78286 exactly.
...
- **Remove it** when the test is about *behavior* and just needs to run to
  ...
  `fork.is_sip_enabled(8037)` budget bumps) and often the `fork` param itself.
...
- **Gas-snapshot tests are gas-sensitive.** If the post asserts a stored `GAS`
  ...
  the stored gas value is exactly what SIP-8037 re-prices and breaks.
- **SIP-8037 caveat:** when you omit `gas_limit` on a test that *measures* an
  ...
  the measurement on SIP-8037 forks (see step 10). Pure-execution opcodes
...
- **Post-state derived from gas/fees.** When the asserted value is a function of
  ...
  `test_sender_balance` (SIP-1559 effective-vs-max price).
...
GAS)` delta stored to slot `0`/`0x64`. That stored value is *why* SIP-8037
...
**SIP-8037 state-gas reservoir — critical.** Omitting `gas_limit` (step 2) on an
SIP-8037 fork *maxes the state-gas reservoir*, so state gas (e.g. account
...
previous_bytes=)`; SIP-3860 init-code words → `fork.gas_costs().CODE_INIT_PER_WORD
...
— rather than a magic number; under SIP-8037 a cold zero->non-zero SSTORE can
...
retains only 1/64 — under SIP-8037 that cannot afford a cold zero→nonzero
...
**Refund-cap derivations need the SIP-7623 kwarg.** The SIP-3529 cap's
...
SIP-684 path): the withheld child grant is consumed, nothing is created,
and under SIP-8037 the new-account state charge is refunded. Useful to
...
**SIP-8037 repriced the code deposit's regular part — boundaries beware.**
...
(SIP-8037) — but only for a fresh target.** When deriving a create-tx
...
**Measuring forwarded gas / the SIP-150 63/64 rule (the `*_gas_ask` shape).**
...
because "available" moves with the SIP-2780 intrinsic change. Make it robust
...
avoids the SIP-8037 state-gas trap. (3) **Derive the expectation from the fork:**
```
...
but creates no account, so **no state gas is charged** under SIP-8037. For a
