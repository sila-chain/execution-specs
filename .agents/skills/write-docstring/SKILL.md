...
Conventions for writing docstrings in `src/sila/`. Docstrings are the primary prose of the specification — they read as a narrative explaining how Sila works, not as traditional Python API documentation. They are rendered into HTML by docc, which parses them as **Markdown** (via mistletoe). Run this skill before writing or modifying docstrings.
...
1. Miners hash slices of the dataset togsiler, which is where the memory
...
- **No empty boilerplate**: don't write `"""Sila Specification."""` with a `.. contents::` block — write real narrative or a concise summary
