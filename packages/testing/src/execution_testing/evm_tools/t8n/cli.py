...
from sila_spec_tools.loaders.fork_loader import ForkLoad
from sila_spec_tools.utils import FatalError, find_fork, parse_hex_or_int
...
def _parse_txs_json_to_testing(
    ...
    for tx_dict in normalized:
        ...
        if "v" not in tx.model_fields_set and tx.secret_key is not None:
            if not fork_supports_sip155 and int(tx.ty) == 0:
                tx.protected = False
            tx.sign()
        txs.append(tx)
