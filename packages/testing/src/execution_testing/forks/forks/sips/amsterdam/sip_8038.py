"""
SIP-8038: State Access Gas Cost Increase.
...
class SIP8038(BaseFork):
    """SIP-8038 class."""

    @classmethod
    def gas_costs(cls) -> GasCosts:
        """
        Return the SIP-8038 state-access gas repricing, layered on top
        of the parent fork's schedule. SIP-8037 then folds its
        state-creation gas into the relevant totals via
        `super().gas_costs()`.
        """
        parent = super(SIP8038, cls).gas_costs()
    ...
    def opcode_gas_map(
        ...
        """
        Return the opcode gas map with the SIP-8038 `EXT*` update:
        `EXTCODESIZE` and `EXTCODECOPY` charge an extra `WARM_ACCESS`
        for the second database read (the code).
        """
        gas_costs = cls.gas_costs()
        opcode_gas_map = dict(super(SIP8038, cls).opcode_gas_map())
    ...
    def _calculate_selfdestruct_gas(
        ...
        """
        gas_cost = super(SIP8038, cls)._calculate_selfdestruct_gas(
            opcode, gas_costs
        )
