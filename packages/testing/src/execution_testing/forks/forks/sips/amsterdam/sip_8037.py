"""
SIP-8037: State Creation Gas Cost Increase.

Harmonization, increase and separate metering of state creation gas costs to
mitigate state growth and unblock scaling.

The companion SIP-8038 state-access repricing lives in its own `SIP8038`
mixin. Because the SIP mixins are ordered by number, `SIP8037` sits
immediately above `SIP8038` in the MRO, so `super().gas_costs()` here
returns the SIP-8038 schedule and this mixin folds its state-creation gas
...
class SIP8037(BaseFork):
    """SIP-8037 class."""

    @classmethod
    def cost_per_state_byte(cls) -> int:
        """
        Return the fixed cost per state byte for SIP-8037.
        """
        return 1530
    ...
    def transaction_total_gas_limit_cap(cls) -> int | None:
        ...
        From SIP-8037 onwards the SIP-7825 `transaction_gas_limit_cap`
    ...
    def system_call_gas_limit(cls) -> int:
        ...
        return super(SIP8037, cls).system_call_gas_limit() + extra

    @classmethod
    def code_deposit_state_gas(cls, *, code_size: int) -> int:
        """Return state gas for code deposit (SIP-8037)."""
        return code_size * cls.cost_per_state_byte()

    @classmethod
    def create_state_gas(cls, *, code_size: int = 0) -> int:
        """Return total state gas for CREATE (SIP-8037)."""
    ...
    def gas_costs(cls) -> GasCosts:
        """
        Return gas costs with the SIP-8037 state-creation gas folded
        into the relevant totals, layered on top of the SIP-8038
        state-access repricing returned by `super().gas_costs()`.
        """
        cpsb = cls.cost_per_state_byte()
        parent = super(SIP8037, cls).gas_costs()
    ...
    def _calculate_selfdestruct_gas(
        ...
        positive balance funds a new account; SIP-8038 (the mixin between
        the base and SIP-8037 in the MRO) adds only the `ACCOUNT_WRITE`
        surcharge. SIP-8037 moves that funding cost to the state-gas
        ...
        cost; the SIP-8038 `ACCOUNT_WRITE` surcharge stays in execution gas.
        """
    ...
    def _calculate_call_state_gas(
        ...
        transfer funds a new account. Before SIP-8037 this was folded
        into the execution CALL cost (SIP-161); under SIP-8037 it is
    ...
    def _calculate_call_gas(
        ...
        """
        Calculate the execution CALL gas cost. The SIP-161 base
        calculation folds `NEW_ACCOUNT` into the execution cost when a
        value transfer funds a new account; SIP-8037 moves that charge
