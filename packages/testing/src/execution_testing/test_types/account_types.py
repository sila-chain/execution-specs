...
import sila.state as spec_state
import sila.state_mpt as spec_state_mpt
from pydantic import PrivateAttr
from sila.crypto.hash import Hash32
from sila.crypto.hash import keccak256 as spec_keccak256
from sila_types.bytes import Bytes, Bytes20
...
class Alloc(BaseAlloc):
    ...
    def get_account_optional(
        ...
        Conforms to `sila.state.PreState.get_account_optional`.
        """
        self._ensure_live()
    ...
    def get_storage(self, address: Bytes20, key: Bytes32) -> U256:
        ...
        Conforms to `sila.state.PreState.get_storage`.
        """
        self._ensure_live()
        account = self.root.get(Address(address))
        if account is None:
    ...
    def get_code(self, code_hash: Hash32) -> Bytes:
        ...
        Conforms to `sila.state.PreState.get_code`.
        """
        self._ensure_live()
    ...
    def compute_state_root(self, block_diff: spec_state.BlockDiff) -> Hash32:
        ...
        Conforms to `sila.state.PreState.compute_state_root`.
    ...
    def apply_diff(self, diff: spec_state.BlockDiff) -> None:
        ...
        # Drop zero-valued storage entries from every account. Sila
