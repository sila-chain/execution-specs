...
def test_iterating_subcall_reserve_includes_state_gas() -> None:
    ...
    SIP-8037 per-byte code deposit) from forwarded execution gas.
    """
...
def test_tx_gas_limit_includes_state_gas_reservoir() -> None:
    """
    Under SIP-8037 ``include_state_gas_reservoir`` adds the per-iteration
...
def test_state_reservoir_lets_tx_gas_exceed_execution_gas_limit_cap() -> None:
    """
    Under SIP-8037 the SIP-7825 transaction gas limit cap binds execution gas
    ...
    assert execution <= cap, "execution gas must respect the SIP-7825 cap"
    assert combined > cap, (
