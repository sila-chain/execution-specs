...
def _history_write(parent_hash: Hash) -> BalAccountChange:
    """Build the SIP-2935 system write of the block's parent hash."""
...
def test_bal_embedded_parent_hash_passes(tmp_path: Path) -> None:
    """
    The SIP-2935 write embeds each side's own parent hash in its BAL; a
...
def test_parent_hash_write_outside_history_contract_raises(
    ...
    only the SIP-2935 history-contract write is masked.
    """
    _write_sibling(
        tmp_path,
        [
