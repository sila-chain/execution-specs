...
class ExecutionSpecsTransitionTool(TransitionTool):
    ...
    def __init__(
        ...
        *,
        ...
        # Defer importing the `sila` package (see `fork_cache` and
        ...
        # pytest-cov starts the worker's coverage session; importing `sila`
    ...
    def version(self) -> str:
        ...
        # Use package metadata rather than `sila.__version__` to avoid
        # importing `sila` here (see `__init__` for why it must stay lazy).
        from importlib.metadata import version

        return version("sila-execution")

    def is_fork_supported(self, fork: Fork) -> bool:
        """Return True if the fork is supported by the tool."""
        from sila_spec_tools.utils import get_supported_forks
    ...
    def _evaluate(
        ...
        *,
        ...
        tracers = None
        if self.trace:
            # TODO: Sip3155 traces still round-trip through tempfile
...
class ExecutionSpecsExceptionMapper(ExceptionMapper):
    ...
    mapping_regex: ClassVar[Dict[ExceptionBase, str]] = {
        ...
        # mapping_substring, and raised from sila.crypto.elliptic_curve
