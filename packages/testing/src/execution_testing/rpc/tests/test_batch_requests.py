...
from execution_testing.rpc import RPCCall, SilRPC
...
@pytest.fixture
def rpc(max_batch_size: int | None) -> SilRPC:
    """Return an `sil` RPC client pointed at a local endpoint."""
    return SilRPC("http://localhost:8545", max_batch_size=max_batch_size)


@pytest.fixture
def post(rpc: SilRPC) -> Iterator[MagicMock]:
...
def test_batch_request_is_chunked(rpc: SilRPC, post: MagicMock) -> None:
...
def test_empty_batch_request_is_not_sent(rpc: SilRPC, post: MagicMock) -> None:
