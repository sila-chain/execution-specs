...
class SilRPC(BaseRPC):
    ...
    def get_block_receipts(
        ...
        """`sil_getBlockReceipts`: Returns every receipt in a block."""
    ...
    def get_transaction_receipts(
        ...
        """
        `sil_getTransactionReceipt` batch: receipts for many transactions.
    ...
    def _transaction_is_known(
        ...
        A retried `sil_sendRawTransaction` whose first delivery succeeded
        ...
        if known:
            logger.warning(
                f"Client answered sil_sendRawTransaction with '{error}' "
...
class DebugRPC(SilRPC):
    ...
    def rewind_head(self, *, block_number: str, block_hash: str) -> None:
        ...
        Prefer gsil's ``debug_setHead`` (takes a block number); if the
        ...
        Note ``debug_setHead`` truncates the chain (gsil), keeping the block
    ...
    def forkchoice_updated(
        ...
        *,
        ...
        if custody_columns is not None:
            # Third parameter of `engine_forkchoiceUpdatedV4` (SIP-8070):
