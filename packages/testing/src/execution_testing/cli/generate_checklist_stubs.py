"""CLI tool to generate mypy stub files for SIPChecklist classes."""
...
def generate_checklist_stubs(output: str | None, dry_run: bool) -> None:
    """
    Generate mypy stub files for SIPChecklist classes.

    This is a development tool that generates .pyi stub files to help mypy
    understand that SIPChecklist classes are callable, fixing type checking
    issues.
    ...
    try:
        ...
        from execution_testing.checklists.sip_checklist import (
            SIPChecklist,
        )

        stub_content = '''"""
Type stubs for SIP checklist - auto-generated.
...
class _CallableChecklistItem:
    ...
    def __new__(cls, *args: Any, sip: Any = ..., **kwargs: Any) -> Any: ...
    @overload
    def __call__(self, func: F) -> F: ...
    @overload
    def __call__(
        self, *, sip: Any = ..., **kwargs: Any
    ) -> pytest.MarkDecorator: ...
    def __str__(self) -> str: ...

'''

        # Generate the main SIPChecklist structure
        lines = generate_class_stub(SIPChecklist, "SIPChecklist", indent=0)
        stub_content += "\n".join(lines) + "\n"
        ...
        click.echo(
            "\n💡 This stub file helps mypy understand that SIPChecklist "
            "classes are callable."
        )
        click.echo(
            "   You can now use @SIPChecklist.Opcode.Test."
            "StackComplexOperations() without type errors!"
        )

    except ImportError as e:
        click.echo(f"❌ Error importing SIPChecklist: {e}", err=True)
        click.echo(
