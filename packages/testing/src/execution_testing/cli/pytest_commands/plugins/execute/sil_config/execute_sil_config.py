"""
Pytest test to verify a client's configuration using `sil_config` RPC endpoint.
"""
...
@pytest.fixture(scope="function")
def expected_sil_config(
    network: NetworkConfig, current_time: int
) -> SilConfigResponse:
    """
    Calculate the current fork value to verify against the client's response.
    """
    return network.get_sil_config(current_time)


def test_sil_config_current(
    sil_config_response: SilConfigResponse | None,
    expected_sil_config: SilConfigResponse,
) -> None:
    """Validate `current` field of the `sil_config` RPC endpoint."""
    assert sil_config_response is not None, (
        "Client did not return a valid `sil_config` response."
    )
    assert sil_config_response.current is not None, (
        "Client did not return a valid `current` fork config."
    )
    expected_current = expected_sil_config.current
    assert sil_config_response.current == expected_current, (
        "Client's `current` fork config does not match expected value: "
        f"{sil_config_response.current.model_dump_json(indent=2)} != "
        f"{expected_current.model_dump_json(indent=2)}"
    )


def test_sil_config_current_fork_id(
    sil_config_response: SilConfigResponse | None,
    expected_sil_config: SilConfigResponse,
) -> None:
    ...
    assert sil_config_response.current.fork_id is not None, (
        "Client did not return a valid `forkId` in the current fork config."
    )
    assert (
        sil_config_response.current.fork_id
        == expected_sil_config.current.fork_id
    ), (
        "Client's `current.forkId` does not match expected value: "
        f"{sil_config_response.current.fork_id} != "
        f"{expected_sil_config.current.fork_id}"
    )


def test_sil_config_next(
    sil_config_response: SilConfigResponse | None,
    expected_sil_config: SilConfigResponse,
) -> None:
    """Validate `next` field of the `sil_config` RPC endpoint."""
    assert sil_config_response is not None, (
        "Client did not return a valid `sil_config` response."
    )
    expected_next = expected_sil_config.next
    if expected_next is None:
        assert sil_config_response.next is None, (
            "Client returned a `next` fork config but expected None."
        )
    else:
        assert sil_config_response.next is not None, (
            "Client did not return a valid `next` fork config."
        )
        assert sil_config_response.next == expected_next, (
            "Client's `next` fork config does not match expected value: "
            f"{sil_config_response.next.model_dump_json(indent=2)} != "
            f"{expected_next.model_dump_json(indent=2)}"
        )


def test_sil_config_next_fork_id(
    sil_config_response: SilConfigResponse | None,
    expected_sil_config: SilConfigResponse,
) -> None:
    ...
    if expected_next is None:
        ...
        if expected_next_fork_id is None:
            assert sil_config_response.next.fork_id is None, (
                "Client returned a `next.forkId` but expected None."
            )
        else:
            received_fork_id = sil_config_response.next.fork_id
...
def test_sil_config_last(
    sil_config_response: SilConfigResponse | None,
    expected_sil_config: SilConfigResponse,
) -> None:
    """Validate `last` field of the `sil_config` RPC endpoint."""
    expected_last = expected_sil_config.last
    assert sil_config_response is not None, (
        "Client did not return a valid `sil_config` response."
    )
    if expected_last is None:
        assert sil_config_response.last is None, (
            "Client returned a `last` fork config but expected None."
        )
    else:
        assert sil_config_response.last is not None, (
            "Client did not return a valid `last` fork config."
        )
        assert sil_config_response.last == expected_last, (
            "Client's `last` fork config does not match expected value: "
            f"{sil_config_response.last.model_dump_json(indent=2)} != "
            f"{expected_last.model_dump_json(indent=2)}"
        )


def test_sil_config_last_fork_id(
    sil_config_response: SilConfigResponse | None,
    expected_sil_config: SilConfigResponse,
) -> None:
    ...
    if expected_last is None:
        ...
        if expected_last_fork_id is None:
            assert sil_config_response.last.fork_id is None, (
                "Client returned a `last.forkId` but expected None."
            )
        else:
            received_fork_id = sil_config_response.last.fork_id
            assert received_fork_id is not None, (
...
def test_sil_config_majority(
    ...
    for exec_client in all_rpc_endpoints.keys():
        ...
        for sil_rpc_target in all_rpc_endpoints[exec_client]:
            try:
                response = sil_rpc_target.config(timeout=5)
                if response is None:
                    logger.warning(
                        f"Got 'None' as sil_config response from "
                        f"{sil_rpc_target}"
                    )
                    continue
            except Exception as e:
                logger.warning(
                    f"When trying to get sil_config from {sil_rpc_target} a "
    ...
    assert len(responses.keys()) == len(all_rpc_endpoints.keys()), (
        "Failed to get an sil_config response from each specified execution "
        f"client. Full list of execution clients is "
        f"{all_rpc_endpoints.keys()} but we were only able to gather "
        f"sil_config responses from: {responses.keys()}\n"
    ...
    for h in client_to_hash_dict.keys():
        ...
        assert client_to_hash_dict[h] == expected_hash, (
            "Critical consensus issue: Not all sil_config responses are the "
            " same!\n"
    ...
    logger.info(
        "All clients returned the same sil_config response. Test has been "
        "passed!"
    )
