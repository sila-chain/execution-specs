"""Unit tests for the `sil_config` execute tests."""
...
@pytest.fixture
def sil_config(network: NetworkConfig, current_time: int) -> SilConfigResponse:
    """
...
def test_fork_config_from_fork(
    sil_config: SilConfigResponse,
    expected_sil_config: SilConfigResponse,
) -> None:
    """Test the `fork_config_from_fork` function."""
    current_config, next_config = (sil_config.current, sil_config.next)
    assert current_config.model_dump(
        mode="json", by_alias=True
    ) == expected_sil_config.current.model_dump(mode="json", by_alias=True), (
        f"Expected {expected_sil_config.current.model_dump_json()} but got "
        f"{current_config.model_dump_json()}"
    )
    assert current_config.fork_id == expected_sil_config.current.fork_id, (
        f"Expected {expected_sil_config.current.fork_id} "
        f"but got {current_config.fork_id}"
    )
    if expected_sil_config.next is not None:
        assert next_config is not None, "Expected next to be not None"
        assert next_config.model_dump(
            mode="json", by_alias=True
        ) == expected_sil_config.next.model_dump(mode="json", by_alias=True), (
            f"Expected {expected_sil_config.next.model_dump_json()} but got "
            f"{next_config.model_dump_json()}"
        )
        assert next_config.fork_id == expected_sil_config.next.fork_id, (
            f"Expected {expected_sil_config.next.fork_id} "
            f"but got {next_config.fork_id}"
        )
    else:
        assert next_config is None, "Expected next to be None"
    if expected_sil_config.last is not None:
        assert sil_config.last is not None, "Expected last to be not None"
        assert sil_config.last.model_dump(
            mode="json", by_alias=True
        ) == expected_sil_config.last.model_dump(mode="json", by_alias=True), (
            f"Expected {expected_sil_config.last.model_dump_json()} but got "
            f"{sil_config.last.model_dump_json()}"
        )
        assert sil_config.last.fork_id == expected_sil_config.last.fork_id, (
            f"Expected {expected_sil_config.last.fork_id} "
            f"but got {sil_config.last.fork_id}"
        )
    else:
        assert sil_config.last is None, "Expected last to be None"
...
def test_fork_ids(
    ...
    assert expected_current_fork_id == sil_config.current.fork_id, (
        f"Unexpected current fork id: "
        f"{sil_config.current.fork_id} != {expected_current_fork_id}"
    )
    if expected_next_fork_id is not None:
        assert sil_config.next is not None, "Expected next to be not None"
        assert expected_next_fork_id == sil_config.next.fork_id, (
            f"Unexpected next fork id: "
            f"{sil_config.next.fork_id} != {expected_next_fork_id}"
        )
    else:
        assert sil_config.next is None, "Expected next to be None"
    if expected_last_fork_id is not None:
        assert sil_config.last is not None, "Expected last to be not None"
        assert expected_last_fork_id == sil_config.last.fork_id, (
            f"Unexpected last fork id: "
            f"{sil_config.last.fork_id} != {expected_last_fork_id}"
        )
    else:
        assert sil_config.last is None, "Expected last to be None"
