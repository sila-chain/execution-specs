...
"""Byte length of a well-formed `custodyColumns` bitmap (SIP-8070)."""
...
class BlobTransaction(BaseExecute):
    ...
    def _update_custody_columns(
        ...
        latest_block = sil_rpc.get_block_by_number("latest")
    ...
    def execute(
        ...
        if self.custody_columns is not None:
            self._update_custody_columns(fork, sil_rpc, engine_rpc)
        ...
        if version <= 2:
            ...
            # V4 (SIP-8070): partial cell matrix, selected by cell_mask
