...
@pytest.fixture
def sip_enabled(fork: Fork) -> bool:
    """Whether the SIP is enabled or not."""
...
def test_create_selfdestruct_same_tx(
    ...
    Behavior should be the same before and after SIP-6780.
    ...
    # the wei sent each time. Also track the sequence of SIP-7708 logs so they
    ...
    for i, sendall_recipient in zip(
        ...
        elif not fork.is_sip_enabled(8246):
            # per SIP-8246
            selfdestruct_contract_current_balance = 0
    ...
    if fork.is_sip_enabled(8246) and selfdestruct_contract_current_balance > 0:
        # per SIP-8246
...
def test_self_destructing_initcode(
    ...
    Behavior is the same before and after SIP-6780.
    ...
    for i in range(call_times):
        ...
        if fork.is_sip_enabled(8246):
            ...
            # SIP-8246 disables self-destruct burns.
    ...
    if fork.is_sip_enabled(7708):
        ...
        # CALLs to the destroyed contract transfer SIL to it.
        for i in range(call_times):
            if i > 0:
                expected_logs.append(
                    transfer_log(
...
def test_selfdestruct_pre_existing(
    ...
    After SIP-6780, the balance should be sent to the send-all recipient
    address, similar to the behavior before the SIP, but the account is not
    deleted.
    ...
    for i, sendall_recipient in zip(
        ...
        # sending to self and the SIP is activated, otherwise the balance is
        # destroyed
        if (
            sendall_recipient != selfdestruct_contract_address
            or not sip_enabled
...
def test_create_multiple_contracts_destroy_one_then_destroy_other_next_tx(
    ...
    Contract B: Does NOT self-destruct in creation tx, attempted destruction
                in subsequent tx (persists after SIP-6780)
    """
    ...
    if sip_enabled:
        # After SIP-6780: Contract B persists (not destroyed in same tx)
        ...
        # Before SIP-6780: Contract B is destroyed in tx2
