...
account is deleted and any balance it still holds is burned; from SIP-8246
...
def finalized(fork: Fork, balance: int) -> Account | None:
    ...
    deleted, or under SIP-8246 kept with only that balance.
    """
    if fork.is_sip_enabled(8246) and balance > 0:
...
def finalized_bal(
    ...
    kept = balance if fork.is_sip_enabled(8246) else 0
...
@SIPChecklist.Opcode.Test.ExecutionContext.Call(sip=[8246])
@SIPChecklist.Opcode.Test.GasUsage.ExtraGas(sip=[8246])
@SIPChecklist.Opcode.Test.Terminating.Scenarios.SubLevel(sip=[8246])
def test_selfdestruct_sequences(
    ...
    """
    Cases 1-12 of the SIP-8246 test list: a contract created in this
    ...
    for step in steps:
        if step == SD_SELF:
            # Before SIP-8246 the opcode burns a same-tx contract's balance.
            if not fork.is_sip_enabled(8246):
                victim_balance = 0
        elif step == SD_OTHER:
            if victim_balance > 0:
...
def test_selfdestruct_clears_nonce_and_storage(
    ...
    """
    Cases 13-16 of the SIP-8246 test list: a contract created in this
    transaction raises its nonce or writes storage before self-destructing.
    It is deleted, or under SIP-8246 has its nonce reset to zero and its
...
@SIPChecklist.Opcode.Test.ExecutionContext.Delegatecall(sip=[8246])
@SIPChecklist.Opcode.Test.ExecutionContext.Delegatecall.Balance(sip=[8246])
@SIPChecklist.Opcode.Test.ExecutionContext.Delegatecall.Code(sip=[8246])
@SIPChecklist.Opcode.Test.ExecutionContext.Delegatecall.Storage(sip=[8246])
@SIPChecklist.Opcode.Test.ExecutionContext.Callcode(sip=[8246])
...
@SIPChecklist.Opcode.Test.ExecutionContext.Staticcall(sip=[8246])
@SIPChecklist.Opcode.Test.ExecutionContext.Staticcall.BanCheck(sip=[8246])
@SIPChecklist.Opcode.Test.ExecutionContext.Staticcall.BanNoModification(
    sip=[8246]
)
@SIPChecklist.Opcode.Test.ExecutionContext.Staticcall.SubCalls(sip=[8246])
@SIPChecklist.Opcode.Test.ExceptionalAbort(sip=[8246])
...
@SIPChecklist.Opcode.Test.ExecutionContext.Initcode.Reentry(sip=[8246])
def test_recreate_in_later_block(
    ...
    SIP-8246 the first account is gone by then, so the second creation
    starts from nothing; from SIP-8246 on it lands on the balance the first
    ...
    def deploy(slot: int) -> Transaction:
        ...
        if fork.is_sip_enabled(7708) and endowment > 0:
    ...
    # Each deployment keeps its endowment under SIP-8246, so the second one
    # finds the first one's balance still there.
    final_balance = 2 * endowment if fork.is_sip_enabled(8246) else 0
...
def test_selfdestruct_to_self_keeps_existing_balance(
    ...
    itself. From SIP-8246 on that balance is never moved at all, so the
