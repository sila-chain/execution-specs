"""
Tests for SIP-150 SELFDESTRUCT operation gas costs.

SIP-150 introduced a gas cost for the SELFDESTRUCT opcode and precise gas
...
def test_selfdestruct_to_self(
    ...
    if fork.is_sip_enabled(7928):
        if same_tx:
            if is_success:
                # per SIP-8246
                if fork.is_sip_enabled(8246) and originator_balance > 0:
                    victim_expectation = BalAccountExpectation(
                        balance_changes=[
                            BalBalanceChange(
                                block_access_index=1,
    ...
    if not is_success:
        ...
        # per SIP-8246
        if fork.is_sip_enabled(8246) and same_tx and originator_balance > 0:
            post = {
                alice: Account(nonce=1),
                caller: Account(nonce=caller_nonce),
                victim: Account(
