"""Helper functions for the SIP-2537 BLS12-381 precompiles tests."""
...
def add_points_g1(point_a: PointG1, point_b: PointG1) -> PointG1:
    ...
    if point_b.x == 0 and point_b.y == 0:
        return point_a
    sila_ecc_point_a = (FQ(point_a.x), FQ(point_a.y))
    sila_ecc_point_b = (FQ(point_b.x), FQ(point_b.y))
    result = add(sila_ecc_point_a, sila_ecc_point_b)
    if result is None:
        return Spec.INF_G1
...
def add_points_g2(point_a: PointG2, point_b: PointG2) -> PointG2:
    ...
    if point_b.x == (0, 0) and point_b.y == (0, 0):
        return point_a
    sila_ecc_point_a = (FQ2(point_a.x), FQ2(point_a.y))
    sila_ecc_point_b = (FQ2(point_b.x), FQ2(point_b.y))
    result = add(sila_ecc_point_a, sila_ecc_point_b)
    if result is None:
        return Spec.INF_G2
...
class BLSPointGenerator:
    ...
    def generate_g1_map_isogeny_kernel_points(cls) -> List[FP]:
        ...
        significantly faster than using `sila_ecc` (200-1000x faster).
