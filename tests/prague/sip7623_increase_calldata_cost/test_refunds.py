"""
Test [SIP-7623: Increase calldata cost](https://sips.sila.org/SIPS/sip-7623).
"""
...
def authorization_list(
    ...
    From SIP-2780 an existing funded authority pays `ACCOUNT_WRITE` and the
...
def top_frame_execution(
    ...
    """Top-frame execution gas charged for authorizations (SIP-8037)."""
    if not authorization_list or not fork.is_sip_enabled(8037):
        return 0
...
def top_frame_state(
    ...
    """Top-frame state gas charged for authorizations (SIP-8037)."""
    if not authorization_list or not fork.is_sip_enabled(8037):
        return 0
...
def max_refund(fork: Fork, refund_type: RefundType) -> int:
    ...
    # Pre-SIP-8037: existing-authority auth refund lands in refund_counter
    # (SIP-3529 1/5 cap). SIP-8037 / SIP-2780 charge AUTH_BASE directly at
    # the top frame with no existing-authority refund.
    if not fork.is_sip_enabled(8037) and auth_existing in refund_type:
...
def intrinsic_gas_data_floor_minimum_delta(
    ...
    On SIP-8037 the SSTORE schedule is higher and existing-authority auths add
    `ACCOUNT_WRITE` and `AUTH_BASE` before the first frame, so the delta must
    cover those too.
    """
    if not fork.is_sip_enabled(8037):
        return 250
    ...
    # above intrinsic + (top_frame + prefix) * 4/5 when the SIP-3529
...
def execution_gas_used(
    ...
    SIP-8037, on top of the authorization top-frame charge).
...
def tx_gas_limit(
    ...
    SIP-7623 floor. When consumption sits below the floor (refund-free
    LESS_THAN cases on SIP-8037), bump the limit up to the floor so the
...
def test_gas_refunds_from_data_floor(
    ...
    Pre-SIP-8037: existing-authority auth refunds flow through
    `refund_counter` (SIP-3529 1/5 cap) togsiler with storage-clear refunds.
