"""Fixtures for the SIP-7623 tests."""
...
from ...cancun.sip4844_blobs.spec import Spec as SIP_4844_Spec
...
def blob_versioned_hashes(ty: int) -> Sequence[Hash] | None:
    ...
    return (
        add_kzg_version(
            [Hash(1)],
            SIP_4844_Spec.BLOB_COMMITMENT_VERSION_KZG,
        )
        if ty == 3
        else None
    )
...
def tx_data(
    ...
    # SIP-7623 token, and byte count grows linearly with `tokens` so
    # both SIP-7623 (per-token) and SIP-7976 (per-byte) floor costs
