...
def authorizations_count(request: pytest.FixtureRequest, fork: Fork) -> int:
    ...
    if request.param == "many_with_execution":
        ...
        # On SIP-8037 the measurement SSTOREs also carry state gas that
...
def _annotate_authorizations_for_top_frame(
    ...
    *,
    ...
    """
    Attach SIP-2780 top-frame charge annotations to each non-skipped
    authorization.
...
def test_gas_cost(
    ...
    Pre-SIP-8037: intrinsic over-charges empty-account auth cost and refunds
    existing authorities via ``refund_counter`` (SIP-3529 1/5 cap).

    SIP-8037 / SIP-2780: intrinsic is execution-only; state-dependent auth
    charges land at the top frame with no existing-authority refund. With
    ``gas_limit`` under the SIP-7825 cap the reservoir is empty, so state
