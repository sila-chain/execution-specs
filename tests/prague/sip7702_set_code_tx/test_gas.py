...
from ...amsterdam.sip8037_state_creation_gas_cost_increase.spec import (
    Spec as Spec8037,
)
...
def gas_test_parameter_args(
    ...
    if include_many:
        ...
        # transaction gas limit cap. Under SIP-8037 the per-auth
        ...
        many_authorizations_count = max_gas // max(
            Spec.AUTH_PER_EMPTY_ACCOUNT, sip_8037_auth_cost
        )
        cases += [
            pytest.param(
                {
...
@pytest.mark.slow()
# TODO[SIP-8037]: discount accounting here uses Prague refund_counter
# mechanics (with the SIP-3529 1/5 cap). On Amsterdam the existing-authority
...
def test_gas_cost(
    ...
    # fifth (SIP-3529) of the total gas used, so we can see the full discount
