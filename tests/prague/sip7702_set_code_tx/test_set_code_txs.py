...
from ...cancun.sip4844_blobs.spec import Spec as Spec4844
from ..sip7685_general_purpose_el_requests.test_multi_type_requests import (
...
def test_signature_s_out_of_range(
    ...
    expected_block_access_list = None
    if fork.is_sip_enabled(7928):
        # High-s rejected pre-load -> authority not in BAL (SIP-7928).
        expected_block_access_list = BlockAccessListExpectation(
            account_expectations={
                tx.sender: BalAccountExpectation(
                    nonce_changes=[
...
def test_nonce_validity(
    ...
    if fork.is_sip_enabled(7928):
        ...
        # is a no-op write and is demoted to storage_reads per SIP-7928.
...
@SIPChecklist.Precompile.Test.CallContexts.SetCode(sip=[7951, 7883, 7823])
...
def test_set_code_to_precompile_not_enough_gas_for_precompile_execution(
    ...
    Redundant from SIP-8037: SIP-8037 replaces the one-dimensional
    ...
    TODO: Add SIP-8037-specific variant in tests/amsterdam/ that
    verifies receipt gas and auth refund under SIP-8037's 2D model.
    """
    ...
    discount = min(
        per_auth_discount,
        intrinsic_gas // 5,  # max discount SIP-3529
    )
...
def test_set_code_to_system_contract(
    ...
    match system_contract:
        case Address(0x00000000219AB540356CBB839CBE05303D7705FA):  # SIP-6110
        ...
        case Address(0x000F3DF6D732807EF1319FB7B8BB8522D0BEAC02):  # SIP-4788
            auth_signer = pre.fund_eoa(
    ...
    match system_contract:
        case Address(0x0000F90827F1C53A10CB7A02335B175320002935):  # SIP-2935
