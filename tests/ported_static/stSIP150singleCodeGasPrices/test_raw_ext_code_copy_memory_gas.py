...
Ported from:
state_tests/stSIP150singleCodeGasPrices/RawExtCodeCopyMemoryGasFiller.json
...
`Op.GAS`. SIP-8038 reprices the cold account access
...
gated on the `is_sip_enabled(8037)` flag (the registered flag that
...
@pytest.mark.ported_from(
    [
        "state_tests/stSIP150singleCodeGasPrices/RawExtCodeCopyMemoryGasFiller.json"  # noqa: E501
    ],
)
...
def test_raw_ext_code_copy_memory_gas(
    ...
    # SIP-8038: cold account repricing plus the extra warm access charged
    # for the opcode's second read (the code). Both terms are 0 before
    # SIP-8038, so the stored cost is unchanged on earlier forks.
    cold_account_delta = gas_costs.COLD_ACCOUNT_ACCESS - 2600
    code_read_delta = cold_account_delta + (
        gas_costs.WARM_ACCESS if fork.is_sip_enabled(8037) else 0
    )
