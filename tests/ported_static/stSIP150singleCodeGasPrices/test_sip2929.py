...
Ported from:
state_tests/stSIP150singleCodeGasPrices/sip2929Filler.yml
...
(`COLD_ACCOUNT_ACCESS`, `COLD_STORAGE_ACCESS`, the SIP-8038 extra
`WARM_ACCESS` for code reads, and `Op.SSTORE` metadata costs), so
every delta is exactly 0 pre-SIP-8037 and tracks future parameter
...
def test_sip2929(
    ...
    # SIP-8038 access-repricing component deltas (each 0 pre-SIP-8037).
    ...
    extra_code_read = gas_costs.WARM_ACCESS if sip_active else 0
    ...
    def _slot_deltas(index: int) -> tuple[int, int, int]:
        """
        Return the (slot0, slot1, slot2) SIP-8038 deltas for an index.
    ...
    def _expect(index: int, base: tuple[int, int, int]) -> dict:
        """Storage dict with each slot bumped by its SIP-8038 delta."""
