...
Ported from:
state_tests/stSIP150singleCodeGasPrices/sip2929-ffFiller.yml

@manually-enhanced: Do not overwrite. The first expect-entry (the
`simple`/NOP case) measures a `CALL` into a contract that
`SELFDESTRUCT`s with an as-yet-untouched (cold) beneficiary. SIP-8038
...
fork's own constant so it is exactly 0 pre-SIP-8038. The other entry
...
def test_sip2929_minus_ff(
    ...
    # SIP-8038 cold account repricing (2600 -> 3000); 0 on earlier forks.
    ...
    #
    #    ; Read so access to that account later won't trigger SIP2929 costs
    #    (balance 0xca11)
    #
