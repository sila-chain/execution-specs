...
derived from SIP-161 rather than pinned at one.
"""
...
# Markers planted at the exact stack depth each SIP-8024 opcode
...
def _base_nonce(fork: Fork) -> int:
    """Return a newly created contract's starting nonce (SIP-161)."""
    return int(fork.is_sip_enabled(161))
...
CASES: dict[Opcodes, Case] = {
    ...
    # --- SIP-8024 immediate-operand stack ops. DUPN[n] copies the
...
def test_opcodes_transaction_init(
    ...
    post[created] = (
        ...
        if case.discarded
            ...
            code=deployed_code,
            # SIP-161 starts a new contract's nonce at one; before it,
            # at zero.
