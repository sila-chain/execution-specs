...
into regular gas; SIP-8038 also reprices the cold account access
...
fork so it is 0 pre-SIP-8037 and tracks parameter changes; bake it
...
def test_precomps_sip2929_cancun(
    ...
    # expect-entry below: SIP-8037 replaces the 25 000 CALL_NEW_ACCOUNT
    # base cost with a state-gas charge that, with an empty reservoir,
    # spills back into regular gas; SIP-8038 separately reprices a cold
    # account access from 2 600 to 3 000. Derive both from the fork so
    # they are 0 pre-SIP-8037 and track parameter changes. `new`
    # entries shift by the account delta, `no` entries by the cold
    # delta, and `all` entries by both.
    new_account_delta = (
        (fork.create_state_gas() - 25000) if fork.is_sip_enabled(8037) else 0
    )
