...
Ported from:
state_tests/stSIP2930/storageCostsFiller.yml

@manually-enhanced: Do not overwrite. This test measures the regular
gas consumed by storage accesses via `Op.GAS`. SIP-8037 moves the
...
def test_storage_costs(
    ...
    # SIP-8037 moves the bulk of storage-write cost into a per-storage
    ...
    # contract; SIP-8037 spills both state-gas charges into regular gas
    # when the reservoir is empty, pushing total consumption over the
    # original 400 000 budget. Bump on SIP-8037; pre-SIP-8037 keeps the
    # original value.
