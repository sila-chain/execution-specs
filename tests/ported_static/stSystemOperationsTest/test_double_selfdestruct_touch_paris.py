...
ran. Extended with a `created_in_tx` arm covering SIP-6780's other
...
def test_double_selfdestruct_touch_paris(
    ...
    if created_in_tx:
        # Deploy it from the transaction's calldata, so SIP-6780 sees a
    ...
    if created_in_tx:
        # SIP-6780 deletes an account created in this same transaction.
        callee = compute_create_address(address=caller, nonce=1)
        post[callee] = Account.NONEXISTENT
    else:
        # SIP-6780 spares a pre-existing account, so the index reaching
