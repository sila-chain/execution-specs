...
SIP-150 63/64 withhold. On the way back up a level must afford its
success-flag store from its 1/64 retention plus whatever its child
returned; levels that cannot (SIP-2200's stipend rule included) halt
...
def predict_recursion_storage(fork: Fork, tx_gas_limit: int) -> dict[int, int]:
    ...
    Descend the self-call chain computing each level's SIP-150 grant,
    ...
    metadata, including SIP-8037 state gas: with a sub-cap gas limit
    ...
    # SIP-2200: any SSTORE with gas_left <= stipend halts exceptionally.
