...
asks for more gas than the transaction provided: the SIP-150 clamp decides
...
SIP-2200 stipend rule would kill the creation), so a failed call and a
...
def test_contract_creation_make_call_that_ask_more_gas_then_transaction_provided(  # noqa: E501
    ...
    # far below the SIP-2200 stipend an SSTORE would require.
    initcode = (
        Op.SSTORE(
            key=CANARY_SLOT,
            value=CANARY,
