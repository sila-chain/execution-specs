...
@manually-enhanced: Do not overwrite. SIP-7928 block access list
expectations added: the precompile is in the list with no balance change
even though the call carried value, and `CALLCODE` sends that value to
the caller itself, so nothing records it either.
"""
...
def test_callcode_ecrecover0_no_gas(
    ...
    # The self-directed value never leaves, so only the transaction's
    # own value shows up here.
