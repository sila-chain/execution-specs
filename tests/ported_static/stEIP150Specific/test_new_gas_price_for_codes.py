"""
Verify the SIP-150 repriced code/account operations in one frame:
...
delegate's budget is derived so its store -- state-priced under SIP-8037
...
`gas_limit - intrinsic - overhead` and shifts with SIP-2780's intrinsic
...
def test_new_gas_price_for_codes(
    ...
    # Fork-derived budget with an SIP-2200 stipend margin for the
