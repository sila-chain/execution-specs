...
account has non-empty code or nonce (SIP-684), and that an account
...
# Every account shape where creation must abort under SIP-684: the
...
# non-empty storage the behavior is undefined in protocol (SIP-7610
...
def test_create_tx_balance_only_target(
    ...
    no storage is not a collision (SIP-684).
    """
    tx = Transaction(
        sender=pre.fund_eoa(),
        ty=tx_type,
