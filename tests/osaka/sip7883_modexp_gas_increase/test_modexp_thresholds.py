...
def test_vectors_from_sip(
    ...
    """Test ModExp gas cost using the test vectors from SIP-7883."""
    state_test(
        pre=pre,
        tx=tx,
        post=post,
...
def test_modexp_legacy_oversized_inputs(
    ...
    lengths. SIP-7823 rejects inputs with any declared length exceeding
    1024 bytes.
    """
    state_test(
        pre=pre,
...
def test_modexp_call_operations(
    ...
    """Test ModExp call related operations with SIP-7883."""
    state_test(
        pre=pre,
        tx=tx,
        post=post,
