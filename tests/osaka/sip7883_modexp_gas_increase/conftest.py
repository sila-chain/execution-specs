"""Shared pytest definitions for SIP-7883 tests."""
...
def exceeds_tx_gas_cap(
    ...
    if fork.is_sip_enabled(8037):
        # SIP-8037: tx.gas can exceed TX_MAX_GAS_LIMIT; excess fills
...
def call_succeeds(
    ...
    SIP-7825, transactions requiring more gas than the cap should fail only if
    unexpected.
    """
...
def precompile_gas(
    ...
    gas.
    """
    spec = Spec if not fork.is_sip_enabled(7883) else Spec7883
    try:
        calculated_gas = spec.calculate_gas_cost(modexp_input)
        if gas_old is not None and gas_new is not None:
            expected_gas = (
                gas_old if not fork.is_sip_enabled(7883) else gas_new
            )
        ...
        return 500 if fork.is_sip_enabled(7883) else 200
