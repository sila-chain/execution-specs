...
from ...byzantium.sip198_modexp_precompile.helpers import (
    ModExpInput,
    ModExpOutput,
)
from ..sip7883_modexp_gas_increase.spec import Spec
...
def test_modexp_boundary(
    ...
    """
    SilaMainnet test at the 1024-byte boundary.

    Tests that the ModExp precompile correctly handles input at the maximum
    allowed length (1024 bytes) per SIP-7823.
    """
...
def test_modexp_over_boundary(
    ...
    """
    SilaMainnet test exceeding the 1024-byte boundary.

    Tests that the ModExp precompile correctly rejects input exceeding the
    maximum allowed length (1024 bytes) per SIP-7823. This proves the SIP
