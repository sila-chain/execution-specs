...
def test_cold_account_access_at_transition(
    ...
    rises across the Amsterdam boundary (2600 -> 3000 on sila-mainnet). The
...
def test_ext_code_surcharge_at_transition(
    ...
    """
    The SIP-8038 ``EXT*`` code-read surcharge appears at the fork. The
...
def test_call_value_cost_at_transition(
    ...
    """
    ``CALL_VALUE`` rises across the boundary (9000 -> 10300 on sila-mainnet,
...
def test_create_base_cost_at_transition(
    ...
    (``OPCODE_CREATE_BASE``: 32000 -> 11000 on sila-mainnet, redefined as
...
def test_selfdestruct_account_write_at_transition(
    ...
    positive balance to an empty account, which is a new SIP-8038
    parameter (0 -> 8000 on sila-mainnet). The constant transition is
...
def test_sstore_write_cost_at_transition(
    ...
    # After the fork the regular portion is the SIP-8038 split:
...
def test_auth_intrinsic_at_transition(
    ...
    ``INTRINSIC_GAS_TOO_LOW`` after, because the SIP-8038 auth intrinsic
    is strictly larger.
    """
    before = fork.fork_at(timestamp=BEFORE_TS)
    after = fork.fork_at(timestamp=AFTER_TS)
