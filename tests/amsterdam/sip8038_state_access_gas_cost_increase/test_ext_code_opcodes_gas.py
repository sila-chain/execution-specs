...
#   - code_read_surcharge: whether SIP-8038 adds the extra WARM_ACCESS read
EXT_OPCODES = [
    pytest.param(
...
def test_ext_code_opcode_gas(
    ...
    assert it matches the SIP-8038 schedule.
    ...
    # SIP-8038 EXTCODESIZE/EXTCODECOPY have a higher cold cost than
...
def test_extcodecopy_nonzero_composes_additively(
    ...
    """
    Verify the SIP-8038 ``EXTCODECOPY`` surcharge composes additively.

    With a non-zero copy size, ``EXTCODECOPY`` charges the account-access
    cost, the SIP-8038 code-read ``WARM_ACCESS`` surcharge, the SIP-150
    ...
    assert expected_gas == (
        access_cost
        + gas_costs.WARM_ACCESS  # SIP-8038 code-read surcharge
...
def test_extcodehash_empty_account(
    ...
    ``EXTCODEHASH`` reads only the account leaf, so SIP-8038 adds no
...
def test_ext_code_double_read_empty_account(
    ...
    """
    Charge the SIP-8038 double-read surcharge on an empty target.
...
def test_extcodecopy_empty_account_composes_additively(
    ...
    yet still charges the account-access cost, the SIP-8038 code-read
    ``WARM_ACCESS`` surcharge, the SIP-150 per-word copy cost
