"""
Tests for the SIP-7702 authorization *regular*-gas repricing under
[SIP-8038: State Access Gas Cost Increase](https://sips.sila.org/SIPS/sip-8038).

SIP-8037 splits each SIP-7702 authorization into a *state* component
(refunded against the state-gas reservoir, covered by the sibling
``sip8037_state_creation_gas_cost_increase`` suite) and a *regular*
...
which on Amsterdam equals ``ACCOUNT_WRITE`` (``8000``) plus the SIP-7702
...
the SIP-8037 suite asserts on the state channel; this suite never
re-asserts it.
"""
...
def _regular_per_auth(fork: Fork) -> int:
    """
    Return the SIP-8038 *regular* intrinsic gas charged per SIP-7702
    authorization, i.e. the total per-auth intrinsic less the SIP-8037
    state portion.
    """
...
def test_auth_regular_intrinsic_magnitude(
    ...
    """
    Assert the SIP-8038 *regular* per-authorization intrinsic magnitude.
    ...
    via ``transaction_intrinsic_state_gas`` and is left to the SIP-8037
    suite.
    """
