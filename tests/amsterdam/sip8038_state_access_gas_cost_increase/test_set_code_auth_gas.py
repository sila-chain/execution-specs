...
``sip8037_state_creation_gas_cost_increase`` and
``sip2780_reduce_intrinsic_tx_gas`` suites; this suite does not
re-assert them.
"""
...
def _execution_per_auth(fork: Fork) -> int:
    """
    Return the *execution* intrinsic gas charged per SIP-7702
    authorization.

    Under SIP-2780 the intrinsic charges only the state-independent
...
def test_auth_execution_intrinsic_magnitude(
    ...
    """
    Assert the SIP-8038 *execution* per-authorization intrinsic magnitude.
...
def test_mixed_validity_multi_auth_receipt_gas(
    ...
    one invalid authorization under the SIP-2780 top-frame charge model.
