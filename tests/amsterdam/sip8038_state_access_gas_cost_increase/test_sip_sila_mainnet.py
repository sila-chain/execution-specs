...
SIP-8038 schedule without re-deriving any per-opcode cost (other files
own those matrices).
"""
...
def test_sstore_zero_to_nonzero(
    ...
    """
    A zero-to-nonzero ``SSTORE`` pays the SIP-8038 storage write and
...
def test_extcodesize(
    ...
    """
    ``EXTCODESIZE`` pays the SIP-8038 account access plus the code-read
...
def test_create_deploys_contract(
    ...
    SIP-8038 schedule and succeeds; the factory records the ``CREATE``
...
def test_selfdestruct_funds_new_account(
    ...
    under the SIP-8038 schedule, forwarding its balance. The new-account
...
def test_auth_installs_delegation(
    ...
    """
    A single valid ``7702`` authorization pays the SIP-8038 auth
