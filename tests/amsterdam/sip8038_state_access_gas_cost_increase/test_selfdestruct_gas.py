"""
Tests for the SIP-8038 [State Access Gas Cost Increase](https://sips.sila.org/SIPS/sip-8038)
``SELFDESTRUCT`` regular-gas dimension.

Under SIP-8038 ``SELFDESTRUCT`` is charged, in its *regular* gas
dimension:
...
the SIP-8037 *state* dimension (`charge_state_gas` in the spec), covered
in ``sip8037_state_creation_gas_cost_increase/test_state_gas_selfdestruct.py``.
...
SIP-6780, a contract not created in the same transaction is not deleted,
...
``sip8037_state_creation_gas_cost_increase/test_state_gas_selfdestruct.py``.
"""
...
from ..sip7708_sil_transfer_logs.spec import transfer_log
...
def _selfdestruct_regular(fork: Fork, *, warm: bool, account_new: bool) -> int:
    """
    Return the SIP-8038 *regular* gas charged by SELFDESTRUCT.

    ``OPCODE_SELFDESTRUCT_BASE + access + (ACCOUNT_WRITE if account_new)``;
    the ``GAS_NEW_ACCOUNT`` account-creation cost is the SIP-8037 state
...
def test_selfdestruct_new_beneficiary_regular_gas(
    ...
    SIP-8037 suite asserts it); here it is funded from the reservoir and
...
def test_selfdestruct_alive_beneficiary_no_account_write(
    ...
    state_test(
        pre=pre,
        # SIP-6780: the pre-deployed destructor is not same-tx-created,
...
def test_selfdestruct_codebearing_zero_balance_beneficiary_no_account_write(
    ...
    zero balance but a non-empty code (``Op.STOP``), so SIP-161 emptiness
...
def test_selfdestruct_self_or_precompile_beneficiary(
    ...
    # SIP-6780: the pre-deployed destructor is not deleted. The self case
...
def test_same_tx_created_selfdestruct_self_burn(
    ...
    """
    SIP-6780: a same-tx-created contract SELFDESTRUCTs to itself, charged
    the warm base only.
    ...
    SIP-8246 removes the SELFDESTRUCT burn, so the self-send is a no-op:
    ...
    is alive at message entry, so SIP-8037 refunds it (the create-tx
    ...
    # intrinsic NEW_ACCOUNT is refunded (SIP-8037).
    ...
    # SIP-8246 removes the SELFDESTRUCT burn: the self-send is a no-op,
...
def test_same_tx_created_selfdestruct_to_fresh_beneficiary(
    ...
    """
    SIP-6780: a same-tx-created contract sends value to a fresh
    ...
    created target is alive at message entry (SIP-8037), while the fresh
