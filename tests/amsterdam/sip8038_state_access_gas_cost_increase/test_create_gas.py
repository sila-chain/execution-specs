...
``COLD_STORAGE_ACCESS`` = 11,000), on top of which the SIP-3860 init
...
and per-byte code deposit charges are the SIP-8037 *state* dimension,
covered in
``sip8037_state_creation_gas_cost_increase/test_state_gas_create.py``.

These tests isolate and assert the SIP-8038 *regular* dimension. At the
...
def test_create_regular_gas(
    ...
    The SIP-8038 *regular* dimension is ``CREATE_ACCESS`` (11,000) plus
    the SIP-3860 init code word cost (2 per word) plus, for ``CREATE2``
    only, an additional keccak word cost (6 per word). The SIP-8037
    account-creation state gas is excluded by subtracting
    ``create_state_gas(0)``.
    """
    gas_costs = fork.gas_costs()
    # The SIP-8038 CREATE regular base equals ACCOUNT_WRITE +
...
def test_create2_keccak_word_delta(
    ...
    SIP-8038 ``CREATE_ACCESS`` base and SIP-3860 word cost.
...
class TestCreateTxGasBoundary:
    """
    Test the contract-creating-transaction gas boundary under SIP-8038.

    Four scenarios pin the boundary, mirroring SIP-3860's
    ``TestContractCreationGasUsage`` but with the SIP-8037/8038 2D gas
    split:
    ...
    def tx_access_list(self) -> List[AccessList]:
        ...
        SIP-7623 floor data cost, mirroring SIP-3860's fixture.
        """
        return [
    ...
    def exact_execution_gas(
        ...
        ``deployment_gas`` is fork-aware: under SIP-8037 it splits the
...
def test_create2_to_occupied_address(
    ...
    # exact CREATE2 target, mirroring the SIP-7610 collision suite.
