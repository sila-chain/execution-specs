...
``COLD_ACCOUNT_ACCESS``), on top of which the SIP-3860 init code
...
These tests isolate and assert the SIP-8038 *execution* dimension. At the
...
def test_create_execution_gas(
    ...
    The SIP-8038 *execution* dimension is ``CREATE_ACCESS`` plus the
    SIP-3860 init code word cost plus, for ``CREATE2`` only, an
    additional keccak word cost. The SIP-8037
...
class TestCreateTxGasBoundary:
    ...
    def exact_execution_gas(
        ...
        Under SIP-2780 the created account's ``NEW_ACCOUNT`` state gas
    ...
    def test_create_tx_gas_boundary(
        ...
        # SIP-2780 the state axis carries the fresh target's top-frame
