...
def build_refund_tx(
    ...
    # SIP-8037: existing authority "refund" adjusts intrinsic_state_gas,
    ...
    # SIP-8037: block gas_used only counts regular gas
    ...
    # SIP-8037: remaining state gas = intrinsic state gas - state gas
...
def test_simple_gas_accounting(
    ...
    # SIP-8037: block gas_used = max(block_regular_gas, block_state_gas)
...
def test_multi_transaction_gas_accounting(
    ...
    """
    # TODO[SIP-8037]: this test's exceed_block_gas_limit branch builds
    ...
    if refund_type == RefundTypes.AUTHORIZATION_EXISTING_AUTHORITY:
        pytest.skip(
            ...
            "two-dimensional block budget post SIP-8037 auth-refund fix"
        )
    ...
    # SIP-8037: block_gas_used = max(sum_regular, sum_state)
...
def test_varying_calldata_costs(
    ...
    if refund_type == RefundTypes.AUTHORIZATION_EXISTING_AUTHORITY:
        if calldata_test_type == (
            ...
            pytest.skip(
                "SIP-7702 auth refund routes through state_gas_reservoir "
...
def test_mixed_gas_regimes(
    ...
    After SIP-8037's calldata-floor alignment, the floor only affects tx-level
    ...
    # SIP-8037: block gas_used counts only regular gas; the SSTORE-set
