"""
Test SSTORE state gas charging under SIP-8037.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "revert",
            marks=SIPChecklist.GasRefundsChanges.Test.ExceptionalAbort.Revertable.Revert(),
        ),
        pytest.param(
            "out_of_gas",
            marks=SIPChecklist.GasRefundsChanges.Test.ExceptionalAbort.Revertable.OutOfGas(),
        ),
        pytest.param(
            "invalid_opcode",
            marks=SIPChecklist.GasRefundsChanges.Test.ExceptionalAbort.Revertable.InvalidOpcode(),
        ),
        pytest.param(
            "upper_revert",
            marks=SIPChecklist.GasRefundsChanges.Test.ExceptionalAbort.Revertable.UpperRevert(),
        ),
    ],
)
@pytest.mark.valid_from("SIP8037")
def test_sstore_nonzero_to_zero(
    ...
    if abort_mode == "success":
        ...
        # Leave enough headroom for SIP-150's 63/64 rule so the child really
...
def test_cross_frame_refund_advance(
    ...
    The tx gas limit sits below the SIP-7825 cap, so the reservoir is
