"""
Test block-level two-dimensional gas accounting under SIP-8037.
...
from ...cancun.sip4844_blobs.spec import Spec as SIP4844_Spec
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            True,
            id="with_log",
            marks=SIPChecklist.BlockLevelConstraint.Test.Content.Logs(),
        ),
    ],
)
@pytest.mark.valid_from("SIP8037")
...
@pytest.mark.valid_from("SIP8037")
def test_block_gas_refund_sip7778_no_block_reduction(
    blockchain_test: BlockchainTestFiller,
    pre: Alloc,
    fork: Fork,
) -> None:
    ...
    (SIP-7778). State gas refund goes to the reservoir and DOES reduce
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            None,
            id="gas_one_below",
            marks=SIPChecklist.BlockLevelConstraint.Test.Boundary.Under(),
        ),
        pytest.param(
            0,
            None,
            id="gas_equal",
            marks=SIPChecklist.BlockLevelConstraint.Test.Boundary.Exact(),
        ),
        pytest.param(
            1,
...
def test_tx_gas_limit_block_boundary(
    ...
    SIP-8037 inclusion rule: ``min(TX_MAX_GAS_LIMIT, tx.gas) <=
    ...
    if tx_type == 1:
        ...
        blob_versioned_hashes = add_kzg_version(
            [Hash(1)], SIP4844_Spec.BLOB_COMMITMENT_VERSION_KZG
        )
...
def test_tx_total_gas_limit_cap(
    ...
    SIP-8037 applies the SIP-7825 cap to execution gas only and caps
...
def test_base_fee_per_gas_follows_dominant_dimension(
    ...
    base fee under the SIP-1559 update.
    """
...
def test_block_2d_inclusion_execution_gate_full_gas_reservation(
    ...
    SIP-8037 measures ``min(TX_MAX_GAS_LIMIT, tx.gas)`` against
