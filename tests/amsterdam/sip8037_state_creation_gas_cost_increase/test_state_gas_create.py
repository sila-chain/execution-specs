"""
Test CREATE and CREATE2 state gas charging under SIP-8037.
...
def test_code_deposit_state_gas_exact_fit_boundary(
    ...
    code). Under SIP-2780 the created account's ``NEW_ACCOUNT`` state gas
    ...
    # SIP-2780, no longer folded into the intrinsic. The RETURN metadata
...
def test_create2_address_collision(
    ...
    non-zero nonce (SIP-684), the collision is detected early and
...
def test_create_tx_below_total_intrinsic(
    ...
    Under SIP-2780 the created account's ``NEW_ACCOUNT`` cost moved out
...
def test_parent_state_gas_after_child_failure(
    ...
    Under SIP-8037 state-gas refunds are LIFO. Gas spilled from
    ...
    if failure_op == Op.INVALID:
        # Simulate runtime gas for HALT under SIP-8037 LIFO refills:
...
def test_nested_create_code_deposit_cannot_borrow_parent_gas(
    ...
    SIP-150 retains 1/64 in the factory, leaving the child short by exactly
...
def test_code_deposit_halt_discards_initcode_state_gas(
    ...
    SIP-3541 0xEF prefix). The exceptional halt reverts all initcode
    ...
    if deposit_fail_mode == "oversized_code":
        ...
        # Return single 0xEF byte: SIP-3541 rejects the code before
...
def test_create_child_halt_refunds_state_gas(
    ...
    Exceptional halts (invalid opcode, SIP-3541 invalid prefix)
    ...
    if failure_mode == "initcode_halt":
        ...
        # Return code starting with 0xEF (SIP-3541 invalid prefix).
...
@pytest.mark.parametrize("fail_mode", ["sip3541", "oog_deposit"])
@pytest.mark.valid_from("SIP8037")
def test_create2_failed_deposit_refunds_storage_state_gas(
    ...
    if fail_mode == "sip3541":
        # return 0xEF -> SIP-3541 rejects the deposited code
        init_code += Op.MSTORE8(0, 0xEF) + Op.RETURN(0, 1)
    else:
...
def test_failed_create_tx_refills_top_frame_new_account(
    ...
    Under SIP-2780 the created account's ``NEW_ACCOUNT`` state gas is
...
def test_create_tx_collision_has_no_net_new_account_charge(
    ...
    Under SIP-2780 the created account's ``NEW_ACCOUNT`` is a top-frame
...
def test_create_tx_collision_refunds_reservoir(
    ...
    SIP-8037 splits `gas_limit` into the capped execution budget and a
...
def test_selfdestruct_in_create_tx_initcode(
    ...
    Under SIP-2780 the created contract's ``NEW_ACCOUNT`` is charged at
...
def test_inner_create_succeeds_code_deposit_state_gas(
    ...
    Under SIP-2780 the outer (tx-level) created account's ``NEW_ACCOUNT``
    ...
    # under SIP-2780; gas_limit must cover it alongside the initcode and
