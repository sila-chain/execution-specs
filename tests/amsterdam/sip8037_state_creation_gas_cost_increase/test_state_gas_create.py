...
def test_create_tx_below_total_intrinsic(
    ...
    SIP-8037 splits the CREATE intrinsic into regular and state
...
def test_nested_create_code_deposit_cannot_borrow_parent_gas(
    ...
    # Invert SIP-150 63/64ths rule: ceil(target_child * 64 / 63).
...
# TODO: Review for bal-devnet-4. If SIP-8037 adopts top-level state gas
# refund (https://github.com/sila/SIPs/pull/11476), the expected block
...
def test_create_account_charge_reduces_child_gas(
    ...
    The target is a pre-existing balance-only leaf, the SIP-8037
...
def test_create_tx_collision_refunds_reservoir(
    ...
    SIP-8037 splits `gas_limit` into the capped regular budget and a
