"""
Test SIP-7623 calldata floor interaction with SIP-8037 state gas.
...
def test_calldata_floor_exceeding_tx_gas_limit_cap(
    ...
    SIP-8037 caps ``max(intrinsic_regular, calldata_floor)`` at
    ``TX_MAX_GAS_LIMIT``. When the SIP-7976 calldata floor crosses the cap
    ...
    if fork.is_sip_enabled(7976):
        # SIP-7976: all bytes contribute 4 floor tokens regardless of
        ...
        # SIP-7623: non-zero bytes contribute 4 tokens, zero bytes 1.
...
def test_calldata_floor_binds_with_reservoir(
    ...
    Large calldata makes the SIP-7976 floor the sender's bill, while an
