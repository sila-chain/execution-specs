...
def test_set_code_tx_below_total_intrinsic(
    ...
    Under SIP-2780 the authorization intrinsic is entirely execution (the
...
def test_existing_account_no_refund(
    ...
    nor ``ACCOUNT_WRITE`` (and, unlike the superseded SIP-8037 behaviour,
...
def test_auth_block_gas_accounting(
    ...
    * ``existing_delegation`` overwrite / clear -- already delegated
    ...
    No branch is refunded (the SIP-8037 over-charge-then-refund is gone).
...
def test_auth_with_calldata_and_access_list(
    ...
    warm the authority under SIP-2780, so the auth charge is unaffected.
    """
    storage = Storage()
...
def test_authorization_exact_state_gas_boundary(
    ...
    Under SIP-2780 the intrinsic is execution-only, so the boundary keys off
...
def test_invalid_auth_no_top_frame_charge(
    ...
    top frame (and, unlike the superseded SIP-8037 model, nothing is
