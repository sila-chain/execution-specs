"""
Test the core SIP-8037 state gas pricing and charge mechanism.
...
def test_pricing_at_various_gas_limits(
    ...
    SIP-8037 prices state gas at a constant `cost_per_state_byte`,
...
def _access_list_over_execution_cap(
    ...
    (the SIP-2930 address cost plus the SIP-7981 floor-token surcharge) and
...
def test_intrinsic_execution_gas_exceeds_cap(
    ...
    SIP-8037 enforces ``max(intrinsic_execution, calldata_floor) <=
...
def test_intrinsic_within_cap_gas_limit_above_cap(
    ...
    SIP-8037 relaxes the SIP-7825 cap on ``tx.gas`` itself; only
...
def test_calldata_floor_enforced_with_state_gas(
    ...
    """
    Test SIP-7623 calldata floor is enforced when SIP-8037 is active.
...
def test_auth_state_gas_scales_with_cpsb(
    ...
    Under SIP-2780 an authorization's state-dependent cost is charged at
