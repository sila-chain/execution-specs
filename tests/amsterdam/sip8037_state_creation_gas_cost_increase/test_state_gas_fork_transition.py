"""
State gas fork transition tests for SIP-8037.
...
constraint activate correctly at the SIP-8037 fork boundary.

Before SIP-8037: no state gas dimension, tx.gas capped at
TX_MAX_GAS_LIMIT (SIP-7825).

At/after SIP-8037: state gas charges apply, tx.gas above
...
pytestmark = pytest.mark.valid_at_transition_to("SIP8037")
...
def test_account_state_gas_at_transition(
    ...
    SIP-8037 splits four charges out of the execution dimension; the
...
def test_sstore_state_gas_at_transition(
    ...
    """
    Test SSTORE state gas activates at the SIP-8037 fork boundary.
...
def test_tx_gas_above_cap_at_transition(
    ...
    """
    Test transaction and block gas constraints at the SIP-8037 transition.

    Before SIP-8037, SIP-7825 rejects any tx with gas > TX_MAX_GAS_LIMIT.
    After SIP-8037 that gas limit is allowed — the excess feeds the state
...
def test_tx_total_gas_limit_cap_at_transition(
    ...
    """
    Test the ``TX_MAX_TOTAL_GAS_LIMIT`` cap on tx.gas at the SIP-8037
    transition.

    Before SIP-8037, SIP-7825 caps tx.gas at TX_MAX_GAS_LIMIT, so a
    transaction at the total cap is rejected. After SIP-8037 the total cap
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            0,
            id="exact",
            marks=SIPChecklist.BlockLevelConstraint.Test.ForkTransition.AcceptedAfterFork(),
        ),
        pytest.param(
            1,
            id="one_above",
...
def test_nonempty_code_deposit_at_transition(
    ...
    for timestamp in (14_999, 15_000):
        ...
        if creation == "transaction":
            ...
            # Smallest grant forwarding exactly child_gas under SIP-150.
