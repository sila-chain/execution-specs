...
expectations below keep each term explicit so the SIP-7981 surcharge is
individually visible:

- SIP-2780 decomposes the flat pre-fork `TX_BASE` into the lowered base
  plus the `COLD_ACCOUNT_ACCESS` recipient charge.
- SIP-8038 reprices the per-address and per-storage-key access list
  charges to the fork's cold access costs.
- SIP-7981 adds four floor tokens per access list byte, charged at
...
pytestmark = pytest.mark.valid_at_transition_to("SIP7981")
...
def test_access_list_intrinsic_across_amsterdam_transition(
    ...
    against a hand-derived per-SIP decomposition, so a calculator
    ...
    # Pre-fork: flat base plus the SIP-2930 per-entry charges; access
    ...
    # Post-fork: SIP-2780 decomposed base and recipient charge, SIP-8038
    # repriced entry charges, and the SIP-7981 byte surcharge.
    surcharge = (
...
def test_access_list_validity_across_amsterdam_transition(
    ...
    the SIP-2780 base reduction, so the post-fork intrinsic is strictly
...
def test_access_list_floor_across_amsterdam_transition(
    ...
    only (access list bytes contribute nothing), post-fork the SIP-7981
    ...
    # Pre-fork (SIP-7623): content-weighted calldata tokens only; the
    ...
    # Post-fork: uniform calldata tokens plus the SIP-7981 access list
    # tokens, anchored on the SIP-2780 decomposed base.
