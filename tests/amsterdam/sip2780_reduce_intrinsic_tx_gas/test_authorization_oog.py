"""
Out-of-gas rollback semantics for SIP-7702 authorizations under
SIP-2780.

At the top frame, SIP-2780 charges each authorization's
...
- **Execution-phase failure** -- the dispatched call itself reverts or
  ...
  applied delegations persist, matching the SIP-7702 "dispatch reverts,
...
def test_set_delegation_oog_charge_point(
    ...
    the halt, so per SIP-7928 they still appear in the block access
...
def test_recipient_charge_oog_rolls_back_delegations(
    ...
    - ``new_account``: value moves to an SIP-161-empty recipient, whose
    ...
    so per SIP-7928 all three must still appear in the block access
...
def test_reservoir_settlement_by_failure_point(
    ...
    A non-zero reservoir requires ``gas_limit`` above the SIP-7825 cap,
    ...
    Togsiler the four pin that the reservoir's fate follows the state
    it paid for: returned in full while nothing survives, consumed to
    the extent the delegations persist.
    """
    cap = fork.transaction_gas_limit_cap()
    assert cap is not None, "SIP-7825 cap expected on this fork"
    gas_costs = fork.gas_costs()
...
def test_delegation_persists_on_execution_oog(
    ...
    delegations survive the halt -- matching the SIP-7702 "dispatch
...
def test_auth_state_charges_survive_dispatch_revert(
    ...
    plain contract) reverts immediately. Per SIP-7702 the applied
...
def test_auth_state_charges_survive_dispatch_halt_with_reservoir(
    ...
    state gas. Here the gas limit exceeds the SIP-7825 cap (allowed --
...
def test_reverted_dispatch_state_gas_counts_toward_block_limit(
    ...
    # ``tx.gas`` (the strict SIP-8037 inclusion rule), charged against
...
def test_recipient_new_account_refilled_on_dispatch_halt_with_reservoir(
    ...
    The gas limit exceeds the SIP-7825 cap so the charge draws from a
...
def test_dispatched_frame_state_gas_still_refills_on_revert(
    ...
    gas altogsiler would overcharge by the 97,920 ``STORAGE_SET``.
    """
    sender = pre.fund_eoa()
