"""
Tests for SIP-2780 Reduce Transaction Intrinsic Cost.

Test gas costs with SIP-2780 for value-moving transactions to:
- EOAs,
- contracts,
- empty accounts,
- the sender itself,
...
def test_value_moving_transactions(
    ...
    The SIP-7708 transfer log is asserted to fire exactly when
...
def test_self_transfer_with_delegated_sender(
    ...
    The SIP's prose skips the delegation-target access for a
...
def test_intrinsic_decomposition_across_tx_types(
    ...
    blob_versioned_hashes = (
        add_kzg_version([Hash(1)], SIP4844_Spec.BLOB_COMMITMENT_VERSION_KZG)
        if tx_type == 3
        else None
    )
...
def test_value_contract_creation_tx(
    ...
    # SIP-2780: the created account's ``NEW_ACCOUNT`` state gas is
    ...
    if tx_reverts:
        ...
        # floor, which SIP-2780 anchors on the create intrinsic base.
        gas_used = max(
            gas_used,
...
def test_value_move_to_precompiles(
    ...
    The top-frame ``NEW_ACCOUNT`` state charge keys solely on SIP-161
    emptiness; a precompile address is not special-cased. The
    ``pre_funded`` parameter exercises both pre-tx states:

    - ``not_funded``: the precompile address is empty per SIP-161, so a
