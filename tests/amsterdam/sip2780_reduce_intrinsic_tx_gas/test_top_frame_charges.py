...
def test_top_frame_state_charge_empty_precompile(
    ...
    """
    An empty precompile recipient is still empty per SIP-161, so a
...
def test_top_frame_new_account_skipped_for_nonce_only_recipient(
    ...
    balance, no code) is not empty per SIP-161, so a value transfer to
    ...
    sender_initial_balance = 10**18
    sender = pre.fund_eoa(sender_initial_balance)
    # Alive via nonce only: not empty per SIP-161 because nonce != 0.
...
def test_top_frame_execution_charge(
    ...
    """
    Recipient is an existing SIP-7702 delegation, so the top-frame
...
def test_initcode_selfdestruct_keeps_top_frame_state_charge(
    ...
    is destroyed at the end of the transaction (SIP-6780 same-tx
    deletion) and its leaf never persists. Deletion itself carries no
    state-gas credit: freeing state is not refunded.

    Where the endowment ends up follows SIP-8246: destruction preserves
    ...
    if beneficiary_kind == "self":
        # SIP-8246: destruction preserves the balance, so a non-zero
...
def test_receipt_status_top_frame_oog_between_successful_txs(
    ...
    makes the status byte load-bearing. (Regression: nimbus-sil1
    ...
    - ``new_account_state_oog``: value transfer to an empty recipient;
      ...
      short.
    - ``delegated_execution_oog``: recipient holds an SIP-7702
