"""
SIP-8024 end-of-code stack underflow regression tests.

When DUPN/SWAPN/EXCHANGE is the last byte of code, the missing immediate
byte decodes to `0` per SIP-8024, and the opcode must still execute with
...
def test_end_of_code_stack_underflow(
    ...
    """
    Test SIP-8024 opcodes at end of code with one fewer stack item than
    required by the zero-decoded immediate.

    Per SIP-8024, `code[pc + 1]` evaluates to `0` when beyond end of code.
    ...
    code = (
        ...
        # end-of-code SIP-8024 opcode, no immediate byte
        + sip8024_opcode
    )
