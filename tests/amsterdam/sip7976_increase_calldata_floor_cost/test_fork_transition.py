...
the old one: SIP-2780 lowers the floor anchor (the decomposed intrinsic
...
pytestmark = pytest.mark.valid_at_transition_to("SIP7976")
...
def expected_floors(fork: TransitionFork, data: bytes) -> tuple[int, int]:
    ...
    Pre-fork (SIP-7623): 10 gas per token, one token per zero byte and
    four per non-zero byte, anchored on the flat `TX_BASE`. Post-fork
    (SIP-7976): 16 gas per token, four tokens per calldata byte
    regardless of content, anchored on the SIP-2780 decomposed base,
...
def test_floor_validity_across_amsterdam_transition(
    ...
    """
    Pin the SIP-7976 validity-threshold change across the boundary.
    ...
    (SIP-7623) floor is accepted in the pre-fork block, but the
    ...
    if scenario == "exact_floors_accepted":
        ...
        # One below the new floor is still rejected post-fork; togsiler
