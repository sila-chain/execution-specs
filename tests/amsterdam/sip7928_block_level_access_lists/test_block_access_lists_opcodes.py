"""
Tests for SIP-7928 Block Access Lists with single-opcode success and OOG
scenarios.
...
https://github.com/paradigmxyz/rsil/issues/17765
...
from tests.frontier.sip2681_limit_account_nonce.spec import Spec as Spec2681

from .spec import ref_spec_7928
from .test_block_access_lists_sip4788 import SYSTEM_ADDRESS
...
class OutOfGasAt(Enum):
    ...
    SIP_2200_STIPEND = "oog_at_sip2200_stipend"
    SIP_2200_STIPEND_PLUS_1 = "oog_at_sip2200_stipend_plus_1"
...
@pytest.mark.parametrize(
    "out_of_gas_at",
    [
        OutOfGasAt.SIP_2200_STIPEND,
        OutOfGasAt.SIP_2200_STIPEND_PLUS_1,
...
def test_bal_sstore_and_oog(
    ...
    ``SSTORE`` clears two gates before the write cost: the SIP-2200
    ...
    if out_of_gas_at == OutOfGasAt.SIP_2200_STIPEND:
        # gas_left == stipend: fails the sentry check outright.
        tx_gas_limit = intrinsic_gas_cost + push_cost + stipend
    elif out_of_gas_at == OutOfGasAt.SIP_2200_STIPEND_PLUS_1:
    ...
    expect_storage_read = out_of_gas_at in (
        ...
        out_of_gas_at == OutOfGasAt.SIP_2200_STIPEND_PLUS_1
...
def test_bal_call_opcode_succeeds_in_static_context(
    ...
    beyond what SIP-214 forbids (only CALL with nonzero value).
    """
    target = pre.deploy_contract(code=Op.STOP)
...
def test_bal_callcode_with_value_in_static_context(
    ...
    SIP-214 explicitly excludes CALLCODE from write-protection.
    """
    target = pre.deploy_contract(code=Op.STOP)
...
def test_bal_call_revert_insufficient_funds(
    ...
    failure happens after delegation resolution. Under SIP-8037 the
...
def test_bal_transient_storage_not_tracked(
    ...
    """
    Test BAL excludes SIP-1153 transient storage (TSTORE/TLOAD).
...
def test_bal_create_nonce_overflow(
    ...
    """
    Test BAL with the factory's nonce at the SIP-2681 boundary.
...
def test_bal_create_existing_target(
    ...
    Under SIP-8037 the account-creation charge is unconditional, so the
...
def test_bal_create2_selfdestruct_then_recreate_same_block(
    ...
    Per SIP-7928 SELFDESTRUCT-in-tx semantics, Tx1's destructed A has no
    ...
    block = Block(
        ...
        expected_block_access_list=BlockAccessListExpectation(
            account_expectations={
                target_a: BalAccountExpectation(
                    # Tx1 destruction (SIP-7928 #165): no nonce/code changes;
...
def test_bal_dirty_account_selfdestruct(
    ...
    - destruction_succeeds: oracle STOPs; per SIP-6780 the same-tx
      selfdestruct fully removes the ephemeral; per SIP-7928 its BAL
