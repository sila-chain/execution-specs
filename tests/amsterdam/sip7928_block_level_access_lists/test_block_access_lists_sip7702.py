"""Tests for the effects of SIP-7702 transactions on SIP-7928."""
...
from ..sip2780_reduce_intrinsic_tx_gas.helpers import (
    AuthorizationAction,
    build_authorization,
)
...
def test_bal_7702_invalid_authority_has_code_authorization(
    ...
    non-empty, non-delegation code (SIP-7702 step-5 rejection, post-load).
    """
...
def test_bal_7702_cross_tx_delegation_then_call(
    ...
    """
    Verify clients apply Tx1's SIP-7702 delegation before later txs CALL
...
def test_bal_selfdestruct_to_7702_delegation(
    ...
    alice_initial_balance = 10**18  # 1 SIL default
...
def test_bal_7702_delegated_create(
    ...
    """
    BAL tracks SIP-7702 delegation indicator write and contract creation.

    Alice sends a type-4 (7702) tx authorizing herself to delegate to
    Deployer code which executes CREATE.

    Under SIP-2780 / SIP-8037 the authorization's state-dependent charges
    ...
    deployer_initial_balance = 10**18  # 1 SIL default
    # Deploy factory
    ...
    # Sender-facing gas_used under SIP-8037: execution + state, no auth refund.
    intrinsic_execution = fork.transaction_intrinsic_cost_calculator()(
        return_cost_deducted_prior_execution=True,
        authorization_list_or_count=authorization_list,
    )
