...
pytestmark = [pytest.mark.valid_at("SIP7708"), getattr(pytest.mark, "sila-mainnet")]
...
def test_simple_transfer_sila_mainnet(
    ...
    """Test that a simple SIL transfer emits a transfer log on sila-mainnet."""
    sender = pre.fund_eoa()
...
def test_call_with_value_sila_mainnet(
    state_test: StateTestFiller,
    pre: Alloc,
) -> None:
    """Test that CALL with value emits a transfer log on sila-mainnet."""
    sender = pre.fund_eoa()
...
def test_selfdestruct_sila_mainnet(
    state_test: StateTestFiller,
    pre: Alloc,
    fork: Fork,
) -> None:
    """Test that SELFDESTRUCT emits a transfer log on sila-mainnet."""
    sender = pre.fund_eoa()
