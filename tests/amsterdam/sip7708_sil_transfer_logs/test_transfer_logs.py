"""
Tests for SIP-7708 Transfer logs.
...
pytestmark = pytest.mark.valid_from("SIP7708")
...
def test_simple_transfer_emits_log(
    ...
    """Test that a simple SIL transfer emits a transfer log."""
...
def test_transfer_to_delegated_account_emits_log(
    ...
    """
    Test that transfer to SIP-7702 delegated account emits correct log.
...
def test_contract_creation_tx_collision(
    ...
    Per SIP-684, contract creation aborts when the target address already
    has non-empty code or nonce. The collision check happens before any
    value transfer, so SIP-7708 emits no Transfer log.
    """
    sender = pre.fund_eoa()
    tx = Transaction(
        sender=sender,
...
def test_call_opcodes_transfer_log_behavior(
    ...
    """
    Test SIL transfer log behavior across all call opcode contexts.
    ...
    if call_opcode == Op.CALL:
        ...
        # log per SIP-7708 ("CALL to a different account").
        post = {}
    else:
...
def test_call_opcodes_insufficient_balance_no_log(
    ...
    of balance — it's a self-transfer exempted by SIP-7708 — so for that
...
def test_selfdestruct_to_system_address(
    ...
    """
    Test SELFDESTRUCT sending SIL to the SIP-7708 system address.
...
def test_inner_create_succeeds_outer_reverts_no_log(
    ...
    factory itself REVERTs. Per SIP-7708 the rollback semantics mirror those
    of CALL: the child log is discarded togsiler with the rest of the
...
def test_nested_calls_log_order(
    ...
    # Forward all gas (`Op.GAS`) rather than a fixed amount: under SIP-8037
...
def test_transfer_with_all_tx_types(
    ...
    """Test that SIL transfers emit logs for all transaction types."""
    recipient = pre.nonexistent_account()
    transfer_amount = 1000
...
def test_selfdestruct_then_transfer_same_block(
    ...
    Tx2: Transfer to the contract triggers SELFDESTRUCT again (code not deleted
         per SIP-6780), sending the received value to beneficiary.
...
def test_selfdestruct_to_self_cross_tx_no_log(
    ...
    Selfdestruct-to-self in Tx2 emits no log per SIP-7708: no Burn
