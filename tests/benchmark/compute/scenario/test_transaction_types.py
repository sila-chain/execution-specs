...
class ReceiverAccountType:
    """Receiver account type for sila transfer benchmarks."""
...
def test_sila_transfers(
    ...
    warm_access: bool,
    sila_transfer_case: Tuple[
        Generator[Address, None, None], Generator[Address, None, None]
    ],
) -> None:
    """
    Single test for sila transfer scenarios.
    ...
    """
    senders, receivers = sila_transfer_case
...
@pytest.mark.parametrize("transfer_amount", [0, 1])
def test_sila_transfers_to_precompile(
    benchmark_test: BenchmarkTestFiller,
    pre: Alloc,
    precompile: int,
    fork: Fork,
    ...
    """Test a block full of sila transfers to a precompile address."""
    # A precompile already exists, so a value transfer pays the SIP-2780
...
def test_auth_transaction(
    ...
    def auth_tx_gas(count: int) -> int:
        ...
        top-frame charges, which have no refunds per SIP-2780.
        """
