...
from sila.utils.numeric import is_prime
...
def test_cache_size_2048_epochs() -> None:
    ...
    cache_size_2048_epochs = json.loads(
        cast(
            bytes,
            pkgutil.get_data("sila", "assets/cache_sizes_2048_epochs.json"),
        ).decode()
    )
...
def test_dataset_size_2048_epochs() -> None:
    ...
    dataset_size_2048_epochs = json.loads(
        cast(
            bytes,
            pkgutil.get_data("sila", "assets/dataset_sizes_2048_epochs.json"),
        ).decode()
    )
...
#
# Gsil DAG related functionalities for fuzz testing
#


def generate_dag_via_gsil(
    gsil_path: str, block_number: Uint, dag_dump_dir: str
) -> None:
    """Generates DAG dataset using gsil for comparison testing."""
    subprocess.call([gsil_path, "makedag", str(block_number), dag_dump_dir])


def fetch_dag_data(dag_dump_dir: str, epoch_seed: bytes) -> Tuple[bytes, ...]:
    """Fetches DAG dataset items from gsil-generated DAG files."""
...
GSIL_MISSING = """gsil binary not found.

Some tests require a copy of the go-sila client binary to generate required
data.
...
def test_dataset_generation_random_epoch(tmpdir: str) -> None:
    """Tests dataset generation against gsil DAG for random epochs."""
    """
    Generate a random epoch and obtain the DAG for that epoch from gsil.
    ...
    NOTE - For this test case to run, it is mandatory for Gsil to be
    installed and accessible
    """
    gsil_path = shutil.which("gsil")
    if gsil_path is None:
        raise Exception(GSIL_MISSING)
    ...
    # data obtained from our implementation with gsil DAG
    for _ in range(500):
