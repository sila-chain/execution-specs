...
The [`State`] class here is the in-memory implementation of the
[`PreState`] protocol used on Sila sila-mainnet: accounts and storage
live in Merkle Patricia Tries and the state root is the MPT
commitment. Other providers, such as databases, witnesses, or other
commitment schemes, are separate implementations of [`PreState`].

[`State`]: ref:sila.state_mpt.State
[`PreState`]: ref:sila.state.PreState
"""
...
from sila.merkle_patricia_trie import (
    EMPTY_TRIE_ROOT,
    Trie,
    copy_trie,
    root,
...
from sila.state import EMPTY_CODE_HASH, Account, Address, BlockDiff, Root
