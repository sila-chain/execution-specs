...
Entry point for the Sila specification.
"""
...
def state_transition(chain: BlockChain, block: Block) -> None:
    ...
    To implement Sila, in theory clients are only required to store the
...
def validate_ommers(
    ...
    the Sila protocol. The maximum amount of ommers is 2 per block and
...
def pay_rewards(
    ...
    create_sila(rewards_state, block_env.coinbase, miner_reward)

    for ommer in ommers:
        # Ommer age with respect to the current block.
        ommer_age = U256(block_env.number - ommer.number)
        ommer_miner_reward = ((U256(8) - ommer_age) * BLOCK_REWARD) // U256(8)
        create_sila(rewards_state, ommer.coinbase, ommer_miner_reward)
...
def process_transaction(
    ...
    # refund gas
    create_sila(tx_state, sender, U256(gas_refund_amount))

    # transfer miner fees
    create_sila(tx_state, block_env.coinbase, U256(transaction_fee))
...
def calculate_block_difficulty(
    ...
    Returns
    -------
    difficulty : `sila.base_types.Uint`
    ...
    # Historical Note: The difficulty bomb was not present in Sila at the
    ...
    # greater than `MINIMUM_DIFFICULTY` on SilaMainnet.
