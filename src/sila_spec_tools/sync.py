...
class BlockDownloader(ForkTracking):
    ...
    def fetch_blocks(
        ...
        if self.gsil:
            ...
            return self.fetch_blocks_sil(first, count)
                ...
                calls.append(
                    {
                        ...
                        "method": "sil_getUncleByBlockNumberAndIndex",
...
class Sync(ForkTracking):
    ...
    def parse_arguments() -> argparse.Namespace:
        ...
        parser.add_argument(
            "--rpc-url",
            help="endpoint providing the Sila RPC API",
        ...
        parser.add_argument(
            "--gsil",
            help="use gsil specific RPC endpoints while fetching blocks",
            action="store_true",
        )
        ...
        parser.add_argument(
            "--sila-mainnet",
            help="Set the chain to sila-mainnet",
            action="store_const",
            dest="chain",
            const="sila-mainnet",
    ...
    def __init__(self) -> None:
        ...
        if not self.options.unoptimized:
            ...
            sila_optimized.monkey_patch(state_path=self.options.persist)
        else:
        ...
        config_str = cast(
            bytes,
            pkgutil.get_data("sila", f"assets/{self.options.chain}.json"),
        ).decode()
        config = json.loads(config_str)

        if self.options.chain == "sila-mainnet":
