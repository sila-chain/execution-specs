...
class ReplaceValue(CodemodArgs, ABC):
    ...
    def _to_args(
        ...
        fully_qualified_name = f"sila.{fork_builder.new_fork}.{qualified_name}"

        fork_root = working_directory / "sila" / fork_builder.new_fork
...
class SetForkCriteria(ReplaceValue):
    ...
    def _replacement(
        ...
        return _Replacement(
            ...
            [
                ("sila.fork_criteria", "Unscheduled"),
                ("sila.fork_criteria", "ByBlockNumber"),
                ("sila.fork_criteria", "ByTimestamp"),
            ],
        )
...
class ClearDocstring(CodemodArgs):
    ...
    def _to_args(
        ...
        init_path = (
            working_directory / "sila" / fork_builder.new_fork / "__init__.py"
        )
        return [
            [
                "codemod",
...
class ForkBuilder:
    ...
    For example, to place `frontier` into `src/sila/frontier`, this value
    would be `src/sila`.
    ...
    def _write_config(
        ...
        config = {
            ...
            "modules": [
                ...
                "sila_spec_tools.new_fork.codemod",
            ],
    ...
    def build(self) -> None:
        ...
        with ExitStack() as exit_stack:
            ...
            package = Path(working_directory.name) / "sila" / self.new_fork
