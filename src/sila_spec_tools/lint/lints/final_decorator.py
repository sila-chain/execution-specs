...
from sila_spec_tools.lint import Diagnostic, Lint
...
def _spec_sources(
    ...
    modules such as `sila.state` and `sila.trace`. The fork packages
    are walked individually because `sila.forks` is a namespace package
    that `walk_packages` does not descend into from the `sila` root. The
    ...
    root = importlib.import_module("sila")
    names += [
        mod_info.name
        for mod_info in pkgutil.walk_packages(root.__path__, "sila.")
    ]
