# Copyright (C) 2022-2023,2026 Sila Foundation
#
...
"""
Plugins for docc specific to the Sila execution specification.
"""
...
class _SilaSortKey:
    ...
    def __lt__(self, other: object) -> bool:
        if isinstance(other, _SilaSortKey):
...
class _SilaPythonSource(PythonSource):
    _sort: Final[int]
    ...
    def listing_order_key(
        self,
    ) -> Tuple[bool, _SilaSortKey, None]:
        return (
            self.index_dir is None,
            _SilaSortKey(self._sort, self.output_path),
            None,
        )


class _SilaListingSource(ListingSource):
    _sort: Final[int]
...
def _find_forks(config: PluginSettings) -> List[Hardfork]:
    forks = config.resolve_path(PurePath("src") / "sila" / "forks")
...
class SilaListingDiscover(ListingDiscover):
    """
...
class SilaPythonDiscover(PythonDiscover):
    """
...
class _PruneReferencesVisitor(Visitor):
    ...
    def _prunable(self, node: Node) -> bool:
        ...
        if parts[:2] != ["sila", "forks"] or len(parts) < 3:
            return False
...
class SilaBuilder(PythonBuilder):
    """
